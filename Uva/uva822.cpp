...
void assign(topic& tp, int t, bool is_wait) {
    int idx = -1;
    int pr = 100;  // priority
    for (int i = 0; i < (int) staffs.size(); i++) {//find the staff idx not busy and with the highest priority
        ...
        if (tpidx < pr) {
            ...
            if (staffs[i].busy < staffs[idx].busy) {//??what if the staff is supposed to pick another request whose priority is even higher than this one
                idx = i;
            }
        }
    }
    ...
    if (idx != -1) {
        ...
        tp.active = t + tp.time;//request theme under staff processing till
        if(is_wait) tp.waiting--;
    }
}

int main() {
    freopen("uva822.txt","r",stdin);
            ...
            for (auto &tp: topics) {
                if(tp.waiting) {
                    for (int k = 0; k < tp.waiting; k++) {
                        assign(tp, t, 1);//assignment to staff
                    }
                }
                if (tp.next == t && tp.reqs > 0) {
                    assign(tp, t, 0);//arrival of new request and assignment to staff
                }
                if (tp.reqs > 0 || tp.active > t) {
                    active++;//number of request themes not sent or under staff processing
                }
            }
            if(!active) {
