...
def main():
    def solve():
        inf = float('inf')
        dst, n, m = list(map(int, input().split()))
        pos, cost, intervals = [0,dst], [inf,0], []
        for _ in range(n):
        ...
        cur = 0
        for i in range(1,m):
            while intervals and intervals[-1][0] < pos[i]:
                l, r = intervals.pop()
                if r <= pos[i]:
                    cur = r 
                else:
                    cur = pos[i]
                    intervals.append((pos[i],r))
            for j in range(i):
                delta = (cur-pos[j])*cost[j] if cur > pos[j] else 0
