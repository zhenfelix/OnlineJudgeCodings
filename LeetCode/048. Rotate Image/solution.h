    ...
    void rotate(vector<vector<int>>& matrix) {
        ...
        while (left < right){
            for (int i = left; i < right; i++){
                swap(matrix[left][i], matrix[right-i+left][left]);
                swap(matrix[right-i+left][left], matrix[right][right-i+left]);
                swap(matrix[right][right-i+left], matrix[i][right]);
                // swap(matrix[i][right], matrix[left][i]);
            }
            left++;
            right--;
        }
